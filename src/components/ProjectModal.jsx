            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div>
                ...
                {project.cast && project.cast.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-xl font-heading font-semibold text-text-primary mb-3">Cast</h3>
                    <div className="space-y-2">
                      {project.cast.map((actor, index) => (
                        <div key={index} className="flex justify-between">
                          <span className="text-text-primary">{actor.name}</span>
                          <span className="text-accent-gold">{actor.role}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {project.crew && (
                  <div className="mb-6">
                    <h3 className="text-xl font-heading font-semibold text-text-primary mb-3">Key Crew</h3>
                    <div className="space-y-2">
                      {Object.entries(project.crew).map(([role, name]) => (
                        <div key={role} className="flex justify-between">
                          <span className="text-accent-gold capitalize">{role}:</span>
              ...
              {/* Right Column - Awards & Gallery */}
              <div>
                {/* Awards */}
                {project.awards && project.awards.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-xl font-heading font-semibold text-text-primary mb-3">Awards & Recognition</h3>
                    <div className="space-y-3">
                      {project.awards.map((award, index) => (
                        <div key={index} className="flex items-start">
                          <svg className="w-5 h-5 text-accent-gold mr-3 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                ...
                {project.gallery && project.gallery.length > 0 && (
                  <div>
                    <h3 className="text-xl font-heading font-semibold text-text-primary mb-3">Gallery</h3>
                    <div className="grid grid-cols-2 gap-3">
                      {project.gallery.map((image, index) => (
                        <div key={index} className="aspect-video bg-gray-800 rounded-lg overflow-hidden group cursor-pointer">
                          <img
                            src={image}
